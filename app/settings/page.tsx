...
export default function SettingsPage() {
  const { user, userProfile, transactions, watchlist } = useStock();
  ...
  useEffect(() => {
    if (userProfile) {
      ...
      setAiProvider(userProfile.ai_provider || 'openai');
      setAiApiKey(userProfile.ai_api_key || '');
    }
  }, [userProfile]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="glass rounded-2xl p-6 flex flex-col items-center text-center">
            ...
            <h3 className="text-sm font-bold text-white mt-4">{fullName || 'Personal Investor'}</h3>
              ...
              {aiProvider !== 'ollama' && (
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-card-border bg-white/5 px-4 py-2.5 text-xs text-white outline-none focus:border-accent-purple/50 focus:ring-1 focus:ring-accent-purple/30 transition-all"
                  />
                  <p className="text-[10px] text-muted/80 leading-normal flex items-start gap-1 mt-1 bg-yellow-500/5 border border-yellow-500/10 p-2.5 rounded-lg">
